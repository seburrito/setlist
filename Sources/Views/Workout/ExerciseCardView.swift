    ...
    private var setsTable: some View {
        VStack(spacing: 0) {
            ...
            ForEach(Array(exercise.sortedSets.enumerated()), id: \.element.persistentModelID) { setIndex, set in
                SetRowView(session: session, exerciseIndex: index, setIndex: setIndex, set: set)
            }
        }
    }
}
...
struct SetRowView: View {
    ...
    private var filled: Bool { set.weight != nil || set.done }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text("\(Analytics.formatWeight(set.ghostWeight)) kg × \(set.ghostReps)")
                .font(.system(size: 13))
            ...
            valueText(Analytics.formatWeight(set.displayWeight))
                .frame(width: 60)
                .contentShape(Rectangle())
                .onTapGesture { session.fillGhost(exerciseIndex: exerciseIndex, setIndex: setIndex) }
            valueText(String(set.displayReps))
                .frame(width: 48)
                .contentShape(Rectangle())
                .onTapGesture { session.fillGhost(exerciseIndex: exerciseIndex, setIndex: setIndex) }
        ...
        .frame(height: 46)
        .padding(.horizontal, 4)
        .background(set.done ? Theme.accent.opacity(0.07) : .clear)
        .clipShape(RoundedRectangle(cornerRadius: 10, style: .continuous))
    }
    ...
    private func valueText(_ text: String) -> some View {
        Text(text)
            .font(.numeral(20))
            .foregroundStyle(set.done ? Theme.accent : (filled ? Color.white : Color.white.opacity(0.3)))
